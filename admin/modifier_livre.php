<?php
    ...
    if (isset($_GET['id']) && is_numeric($_GET['id'])) {
        $id_livre_modifier = $_GET['id'];
        $livre = getLivreDetails($pdo, $id_livre_modifier);
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (empty($errors)) {
            if (modifierLivre($pdo, $id_livre_modifier, $titre, $auteur, $isbn, $date_publication, $nombre_exemplaires)) {
                $success = true;
                $message = "<div class='success-message'>Le livre a été modifié avec succès. <a href='gestion_livres.php'>Retour à la gestion des livres</a></div>";
            } else {
...
<main>
    <h1>Modifier le livre: <?php echo htmlspecialchars($livre['titre']); ?></h1>
    ...
    <?php if ($livre): ?>
        <form method="post">
            ...
            <div class="form-group">
                ...
                <input type="text" id="isbn" name="isbn" value="<?php echo htmlspecialchars($livre['isbn']); ?>">
            </div>
            <div class="form-group">
                <label for="date_publication">Date de publication:</label>
                <input type="date" id="date_publication" name="date_publication" value="<?php echo htmlspecialchars($livre['date_publication']); ?>">
            </div>
            <div class="form-group">
                <label for="nombre_exemplaires">Nombre d'exemplaires:</label>
                <input type="number" id="nombre_exemplaires" name="nombre_exemplaires" value="<?php echo htmlspecialchars($livre['nombre_exemplaires']); ?>" min="1" required>
            </div>
